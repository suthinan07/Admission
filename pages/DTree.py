...
try:
    df = pd.read_csv('adm_data.csv')   # ✅ เปลี่ยนชื่อไฟล์
    ...
    # --- Data Preparation ---
    target_column = 'Result'   # ❗ ต้องเปลี่ยนถ้าไฟล์คุณใช้ชื่ออื่น
    ...
    # สร้าง input ตาม feature จริง ๆ
    user_input = {}
    ...
    tree.plot_tree(dtree, feature_names=features, class_names=True, ax=ax, filled=True, rounded=True, fontsize=10)
    st.pyplot(fig)

except FileNotFoundError:
    st.error("❌ *ไม่พบไฟล์ 'adm_data.csv'* กรุณาตรวจสอบว่าไฟล์อยู่ในโฟลเดอร์เดียวกับโค้ด")
